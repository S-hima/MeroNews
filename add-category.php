...
<script>
  ...
  const titleInput = document.getElementById('category_title');
  ...
  form.addEventListener('submit', (e) => {
    ...
    // Regex to check for consecutive or repeating patterns of special characters
    const specialCharPattern = /([-!@#$%^&*(),.?":{}|<>/\\]{2,})|([-!@#$%^&*(),.?":{}|<>/\\]{1}[-!@#$%^&*(),.?":{}|<>/\\]{1})/;
    if (specialCharPattern.test(title)) {
      errorTitle.textContent = 'Title must not contain consecutive or repeating patterns of special characters.';
      valid = false;
    } else {
